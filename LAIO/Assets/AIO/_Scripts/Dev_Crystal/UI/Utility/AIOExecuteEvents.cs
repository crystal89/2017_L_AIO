...
/// 参考Unity - UISystem
/// </summary>
public static class AIOExecuteEvents
{
    ...
    private static void GetChildrenEventChain(GameObject root, IList<Transform> eventChain)
    {
        eventChain.Clear();
        if (root == null)
            return;
    ...
    private static readonly AIOObjectPool<List<IEventSystemHandler>> s_HandlerListPool = new AIOObjectPool<List<IEventSystemHandler>>(null, l => l.Clear());
    ...
    public static void ExecuteHierarchyChildren<T>(GameObject root, BaseEventData eventData, EventFunction<T> callbackFunction) where T : IEventSystemHandler
    {
        GetChildrenEventChain(root, s_InternalTransformList);

        for (var i = 0; i < s_InternalTransformList.Count; i++)
        {
            var transform = s_InternalTransformList[i];
            Execute(transform.gameObject, eventData, callbackFunction);
        }
    }
    ...
    private static void GetEventList<T>(GameObject go, IList<IEventSystemHandler> results) where T : IEventSystemHandler
    {
        ...
        var components = AIOListPool<Component>.Get();
        ...
        AIOListPool<Component>.Release(components);
